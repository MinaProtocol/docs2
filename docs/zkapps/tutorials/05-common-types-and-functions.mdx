...
`toFields()` gives 2 Fields for a private key and 3 Fields for a signature, as the output shows. Inside a circuit, the elliptic curve scalars in a private key and a signature are most efficiently represented as an array of bits, and the bit length of these scalars is 255.
...
points8 JSON: {"points":[{"x":"0","y":"0"},{"x":"1","y":"10"},{"x":"2","y":"20"},{"x":"3","y":"30"},{"x":"4","y":"40"},{"x":"5","y":"50"},{"x":"6","y":"60"},{"x":"7","y":"70"}]}
```
...
```sh
cd examples/zkapps/05-common-types-and-functions
npm install
npm test
```

It asserts each value that the console-output blocks on this page print, from
`num1 === num2: true` to `value for key 100: 50`. It also runs both contracts
on a local blockchain:

- `BasicMerkleTreeContract` accepts an increment of `9` and **rejects** an
  increment of `10`. After an update, the contract root and the local tree
  root are equal.
- `LedgerContract` moves `12` to an existing recipient and to a new recipient.
  It **rejects** a transfer signed by a different key, a transfer of the full
  balance, and a new recipient that claims a balance that is not `0`.

`main.ts` compiles both contracts and takes about two minutes. The test suite
turns proofs off and takes about 20 seconds.
