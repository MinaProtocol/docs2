...
When the transaction is run on chain, these account updates are checked by the Mina network and applied if valid. Each `AccountUpdate` class includes either a proof corresponding to the verification key in the zkApp account on-chain or a signature corresponding to the zkApp address. In this transaction, the fee update and the two deploy updates are authorized by signatures, and the two init updates by proofs. After deployment, `proofsOnlyZkApp` accepts only proof authorization.
...
View the [account updates visualization](/img/tutorial-10-txn1.png) again.
...
The example project in this repository carries a test suite for the code on
this page:

```sh
cd examples/zkapps/10-account-updates
npm install
npm test
```

It asserts the account update structure that the visualizations on this page
show:

- The deploy transaction has the 5 account updates in the order that this page
  lists. The fee update takes 2 MINA from the deployer. Each deploy update has
  a nonce precondition, and each init update has a `provedState: false`
  precondition. `proofsOnlyZkApp` gets proof-only permissions and
  `secondaryZkApp` gets the default permissions.
- `add(4)` is a parent update that sets `appState[0]` to `5`, with a child
  `incrementCalls()` update that sets `appState[1]` to `1`.
- `callSecondary()` is a parent update with two children, in this order:
  `secondaryZkApp.add()` and `incrementCalls()`. It sets `num` to `12`, and
  the secondary `num` becomes `17`.

It also checks that the network **rejects** these transactions, and that the
state and the balances do not change:

- A second call to `init()`.
- An update that is signed with the private key of `proofsOnlyZkApp`. The same
  update on `secondaryZkApp` is accepted.
- An `add()` transaction whose state precondition is no longer true.
- A proved transaction that the fee payer did not sign.

`main.ts` compiles both contracts when proofs are on. The test suite turns
proofs off.
