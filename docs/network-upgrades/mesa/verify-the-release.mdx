...
Two packages supply the programs. Replace `4.x.x` with the published Mesa version, `3.4.x` with the pre-fork release version, and `<network>` with the network you forked from.
...
sudo apt-get install mina-create-<network>-prefork-genesis-ledger=3.4.x
```
