...
Two packages supply the programs. Replace `4.x.x` with the published Mesa version, and `<codename>` with your distribution (`bullseye`, `bookworm`, `focal`, `jammy`, or `noble`).

```bash
# The Mesa daemon, which also carries the verification tooling
sudo apt-get install mina-mesa=4.x.x

# The pre-fork genesis generator, used to build the reference ledgers
sudo apt-get install mina-create-devnet-prefork-genesis-ledger=3.4.x
```
...
| Program | Package |
| --- | --- |
| `mina-verify-packaged-fork-config` | `mina-mesa` |
| `mina` | `mina-mesa` |
| `mina-create-genesis` | `mina-mesa` |
| `mina-hf-create-runtime-config` | `mina-mesa` |
| `mina-rocksdb-scanner` | `mina-mesa` |
| `mina-create-prefork-genesis` | `mina-create-<network>-prefork-genesis-ledger` |

The pre-fork generator is a separate package because it is built from the pre-fork release, not from Mesa. Install the one that matches the chain you forked from — `mina-create-devnet-prefork-genesis-ledger` for a devnet fork, `mina-create-mainnet-prefork-genesis-ledger` for a mainnet fork.
