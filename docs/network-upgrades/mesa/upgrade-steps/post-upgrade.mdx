...
mina-mesa client status | grep -E "^(Block height|Block producers running|Coinbase receiver):"
```
...
If `mina-mesa` is not on `$PATH` (for example, when running from a Docker image without the automode wrapper), invoke `mina client status` instead, or call it directly inside the container:
