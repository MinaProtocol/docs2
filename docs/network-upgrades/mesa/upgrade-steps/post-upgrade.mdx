...
# Check node status (use mina-mesa if running automode packages)
...
```bash
# Check your recent block production
mina-mesa client status | grep "Block"
```
...
mina-mesa client status | grep -i snark
```
...
Use the `mina-archive-hardfork-toolbox` to verify the migration. All commands require `--postgres-uri postgresql://<user>:<password>@<host>:<port>/<db>`. See the [Archive Upgrade](/network-upgrades/mesa/archive-upgrade#verification-with-the-archive-hardfork-toolbox) page for full toolbox documentation.
...
For full details, see the [Mesa release notes](https://github.com/MinaProtocol/mina/releases/tag/4.0.0).
