...
# Step 2: Add the repository with the dryrun channel.
# packages.o1test.net is unsigned, hence [trusted=yes].
echo "deb [trusted=yes] http://packages.o1test.net $(lsb_release -cs) dryrun" | \
...
Live status, chain data, and configuration for Mesa Trail. The network is identified as `mesa-rc` in infrastructure paths and explorer URLs.

### Upgrade Status Tracker

The **[Mesa Upgrade Status page](https://mesa-upgrade-tracker.minaprotocol.com/)** is the live source of truth during the upgrade. It publishes the current phase, the fork timeline (`stop-transaction-slot`, `stop-network-slot`, Mesa genesis timestamp), the artifact matrix as builds are promoted, announcements, and a running FAQ / known-issues list. Check it before and during the fork window — the slot values and release tags land there first.
...
- Watch the [Mesa Upgrade Status page](https://mesa-upgrade-tracker.minaprotocol.com/) for the fork timeline and artifact promotions
