...
Both modes rely on two critical slot numbers baked into the stop-slot release (4.0.0):
...
When you install the automode package (or use the auto-hardfork Docker image), all three components are installed together.

### The Dispatcher

The dispatcher (`mina-dispatch`) is a shell script that acts as a transparent wrapper around the real `mina` binary. When you run any `mina` command, the dispatcher decides which binary to execute based on a simple check:
...
This limitation exists because, for non-daemon commands, the dispatcher does not receive the config directory location as an argument. Without access to the config directory, it cannot check for the `activated` state file and therefore cannot determine whether the node is running Berkeley or Mesa. The `daemon` subcommand is the only one that passes `--config-directory`, which is why it is the only supported subcommand.
:::

The dispatcher **only supports the `daemon` subcommand**. Any other subcommand (e.g., `client status`, `accounts list`, `ledger export`) will fail with an error:

```
mina-dispatch ERROR: unsupported subcommand 'client' for automatic hardfork handling
```

It is the user's responsibility to invoke the correct version-specific binary for non-daemon commands:
...
The `--version` flag is the one exception — it is passed through the dispatcher without processing.
...
If you run your node with Docker, use the **auto-hardfork** image:
...
As described in the [Dispatcher Limitations](#dispatcher-limitations) section, the dispatcher only supports the `daemon` subcommand. For any other command — including `client status`, `accounts list`, `ledger export`, and all other CLI operations — **you must use the version-specific binary directly**:
